  ...
  const adminId = req.adminId;
    ...
    try {
        ...
        if(!allowedFormat.includes(image.mimetype)){
            return res.status(400).json({errors: "invalid file type"});
        }
...
export const courseDetail = async (req,res) => {
    ...
    try {
        ...
        res.status(500).json({errors: "error in getting course details"})
         console.log("error in course detail", error);
    }
}
...
export const buyCourses = async (req,res) => {
    ...
    try {
        ...
        if (existingPurchase) {
          return res.status(400).json({ error: "User has already purchase this course"}) 
        }
