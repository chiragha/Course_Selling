...
function Home() {
  ...
  const [loggedIn, setLoggedIn] = useState(false)
     ...
     const handleLogout = () =>{
         try {
            const res = axios.get("http://localhost:4001/api/v1/user/logout",{
     ...
     <div className=' text-black container mx-auto'>
      ...
      <header className='flex items-center justify-between p-6'>
        ...
        <div className='space-x-4'>
           {loggedIn ? (
             <button onClick={handleLogout} className='bg-transparent text-amber-800 py-2 px-4 border border-amber-500 rounded'>Logout</button>
             ) : 
           (
            <>
      ...
      <section className='text-center py-20'>
        ...
        <div className='space-x-4 mt-8'>
             <Link to={"/courses"} className='bg-green-500 text-white duration-300  rounded font-semibold py-3 px-6 hover:bg-amber-300 hover:text-amber-950 cursor-pointer'>Explore Courses</Link>
        ...
        <Slider {...settings}>
          {courses.map((course) => (
            <div key={course._id} className='p-4'>
              ...
              <div className='bg-gradient-to-b from-cyan-400 to-cyan-100
              rounded-lg overflow-hidden'>
                <img src={course.image?.url} alt={course.title || ''} className='w-full h-32 object-contain' />
            
              <div className='p-6 text-center'>
                <h3 className='font-semibold text-black'>{course.title}</h3>
               <button className="bg-green-500 text-gray-700 duration-300 rounded-2xl font-semibold p-2 hover:bg-amber-300 hover:text-amber-950 cursor-pointer">Explore Now</button>
              </div>
              </div>
            </div>
            </div>
